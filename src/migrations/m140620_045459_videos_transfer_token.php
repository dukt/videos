...
class m140620_045459_videos_transfer_token extends BaseMigration
{
    ...
    {
        Videos::$plugin->getOauth()->saveToken($handle, $token);
    }
        ...
        try {
            ...
            if(class_exists('OAuth\OAuth2\Token\StdOAuth2Token'))
            {
                ...
                if($row)
                {
                    ...
                    {
                        Craft::trace('Token error.', __METHOD__);
                    }
                }
                else
                {
                    Craft::trace('Token record error.', __METHOD__);
                }
            }
            else
            {
                Craft::trace('Class error.', __METHOD__);
            }
        }
        catch(\Exception $e)
        {
            Craft::trace($e->getMessage(), __METHOD__);
        }
    }
}
